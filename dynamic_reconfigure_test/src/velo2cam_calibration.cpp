...
void sensor1_callback(
    const velo2cam_calibration::ClusterCentroids::ConstPtr sensor1_centroids) {
  if (DEBUG) ROS_INFO("sensor1 callback");
  ...
  if (!S1_WARMUP_DONE) {
    ...
    if (DEBUG) ROS_INFO("return1");
    return;
  }

  if (!S2_WARMUP_DONE) {
    if (DEBUG) ROS_INFO("return2");
    return;
  }
...
void sensor2_callback(
    velo2cam_calibration::ClusterCentroids::ConstPtr sensor2_centroids) {
  if (DEBUG) ROS_INFO("sensor2 callback");
  ...
  if (!S2_WARMUP_DONE && S1_WARMUP_DONE) {
    ...
    if (DEBUG) ROS_INFO("return5");
    return;
  } else if (!S2_WARMUP_DONE) {
    if (DEBUG) ROS_INFO("return6");
    return;
  }
  if (DEBUG) ROS_INFO("sensor2 (%s) pattern ready!", sensor2_frame_id.c_str());
  if (DEBUG) ROS_INFO("sensor2 (%s) pattern ready!", "test");
  ...
  if (is_sensor2_cam) {
    if (DEBUG) ROS_INFO("is_sensor2_cam");
    std::ostringstream sstream;
...
int main(int argc, char **argv) {
  ...
  // Kangrok Made: cfg file save for dynamic_reconfigure_test package for points2image packages!
  ofstream fout;
    ofstream fout2;
  ...
  fout.open("..//..//dynamic_reconfigure_test//cfg//test1.cfg"); 
  ...
  fout2.open("//home//hmcl1//catkin_ws_LCC//src//dynamic_reconfigure_test//cfg//test2.cfg"); 
	fout << "# !/usr/bin/env python" << endl;
	fout << "\n" << endl;
	fout << "from dynamic_reconfigure.parameter_generator_catkin import *" << endl;
	fout << "\n" << endl;
